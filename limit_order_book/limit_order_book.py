...
class ModificationConfirmation():
    def __init__(self, order, new_volume, old_volume, side):
    ...
    def __repr__(self) -> str:
        return f'ModificationConf(agent_id={self.order.agent_id}, order_id={self.order.order_id}, new_volume={self.new_volume}, old_volume={self.old_volume}, time={self.order.time}, side={self.side})'
...
class LimitOrderBook:
    ...
    def _logging(self, order, msg=None):
        # WARNING: logging might not work as expected for only_volumes mode ! 
        ...
        if order.time > self.time:
            ...
            # WARNING: does not work for the volume only mode 
            # order.type == 'cancellation_by_price_volume' will just create a bunch of cancellation messages
            if order.type == 'cancellation':
                if msg.side == 'bid':
                    self.data.cancellation_limit_buy.append(msg.volume)
                    self.data.cancellation_limit_sell.append(0)
                else:
                    self.data.cancellation_limit_sell.append(msg.volume)
                    self.data.cancellation_limit_buy.append(0)
            
            # this should never happen. cancellation by price and volume is just a bunch of cancellation and modification messages
            if order.type == 'cancellation_by_price_volume':
                # this should be handled by cancellation and modification messages
                pass 
                # raise NotImplementedError("cancellation by price volume logging not implemented yet")
            
            if order.type == 'modification':
                # pass 
                filled_volume = msg.old_volume - msg.new_volume
                assert filled_volume > 0, "filled volume must be positive"
                if msg.side == 'bid':
                    self.data.cancellation_limit_buy.append(filled_volume)
                    self.data.cancellation_limit_sell.append(0)
                else:
                    self.data.cancellation_limit_sell.append(filled_volume)
                    self.data.cancellation_limit_buy.append(0)
    ...
    def process_order(self, order, msg=None):
        """
    ...
    def modification(self, order):
        ...
        side = self.order_map[order.order_id].side 
        return ModificationConfirmation(order=order, new_volume=order.volume, old_volume=old_volume, side=side)
