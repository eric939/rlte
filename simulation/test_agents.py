...
from agents import ExecutionAgent, LinearSubmitLeaveAgent, TestAgent, MarketMakingAgent
from queue import PriorityQueue

def test_market_making_agent_limit_fills():
    # testing limit fills. full and partial fills. 
    MA = MarketMakingAgent(volume=10, agent_id='market_making_agent')
    LOB = LimitOrderBook(['market', 'market_making_agent'])
    orders = []
    ...
    orders.append(LimitOrder(side='ask', agent_id='market', price=103, volume=20, time=5))
    [LOB.process_order(order) for order in orders]
    orders = []
    orders.append(LimitOrder(side='ask', agent_id='market_making_agent', price=101, volume=2, time=6))
    orders.append(LimitOrder(side='bid', agent_id='market_making_agent', price=99, volume=3, time=7))
    msg_list = [LOB.process_order(order) for order in orders]
    MA.update_position_from_message_list(msg_list)
    # 
    assert MA.active_volume_ask == 2
    assert MA.active_volume_bid == 3
    assert MA.inventory == 0
    # market buy order. should clear the market making order 
    order = MarketOrder(side='ask', agent_id='market', volume=10+2+5, time=8)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    # fill on the ask side (limit sell order)
    assert MA.inventory == -2
    # fill of limit sell at this price  
    assert reward == 101*2  
    assert MA.active_volume_ask == 0
    assert MA.active_volume_bid == 3
    # also hitting the bid side 
    order = MarketOrder(side='bid', agent_id='market', volume=10+10+2, time=9)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    assert reward == -99*2
    # buying so increasing the inventory 
    assert MA.inventory == 0 
    assert MA.active_volume_ask == 0
    assert MA.active_volume_bid == 1
    # 
    order = MarketOrder(side='bid', agent_id='market', volume=1, time=10)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    assert reward == -99*1
    assert MA.inventory == 1
    assert MA.active_volume_ask == 0
    assert MA.active_volume_bid == 0    
    return None

def test_making_agent_cancellations():
    MA = MarketMakingAgent(volume=10, agent_id='market_making_agent')
    LOB = LimitOrderBook(['market', 'market_making_agent'])
    orders = []
    orders.append(LimitOrder(side='bid', agent_id='market', price=100, volume=10, time=0))
    ...
    [LOB.process_order(order) for order in orders]
    orders = []
    orders.append(LimitOrder(side='ask', agent_id='market_making_agent', price=101, volume=5, time=2))
    orders.append(LimitOrder(side='bid', agent_id='market_making_agent', price=99, volume=5, time=3))
    msg_list = [LOB.process_order(order) for order in orders]
    MA.update_position_from_message_list(msg_list)
    ID = msg_list[0].order_id
    # 
    assert MA.active_volume_ask == 5
    assert MA.active_volume_bid == 5
    assert MA.inventory == 0
    # cancellation of part of the limit sell order 
    order = CancellationByPriceVolume(side='ask', agent_id='market_making_agent', price=101, volume=3, time=4)
    fill = LOB.process_order(order)
    MA.update_position_from_message_list([fill])
    assert MA.active_volume_ask == 2
    assert MA.active_volume_bid == 5
    assert MA.inventory == 0
    # cancellation of the limit buy order 
    order = CancellationByPriceVolume(side='bid', agent_id='market_making_agent', price=99, volume=5, time=5)
    fill = LOB.process_order(order)
    MA.update_position_from_message_list([fill])
    assert MA.active_volume_ask == 2
    assert MA.active_volume_bid == 0
    assert MA.inventory == 0
    # 
    order = Cancellation(agent_id='market_making_agent', order_id=ID, time =6)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    assert reward == 0
    assert MA.active_volume_ask == 0
...
def test_market_making_agent_clear_position():
    MA = MarketMakingAgent(volume=10, agent_id='market_making_agent')
    LOB = LimitOrderBook(['market', 'market_making_agent'])
    orders = []
    orders.append(LimitOrder(side='bid', agent_id='market', price=100, volume=10, time=0))
    orders.append(LimitOrder(side='ask', agent_id='market', price=101, volume=10, time=1))
    orders.append(LimitOrder(side='ask', agent_id='market', price=102, volume=10, time=1))
    [LOB.process_order(order) for order in orders]
    orders = []
    orders.append(LimitOrder(side='ask', agent_id='market_making_agent', price=101, volume=5, time=2))
    orders.append(LimitOrder(side='bid', agent_id='market_making_agent', price=99, volume=5, time=3))
    ...
    MA.update_position_from_message_list(msg_list)
    # 
    order = MarketOrder(side='ask', agent_id='market', volume=10, time=10+4)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    assert MA.inventory == -4
    assert reward == 101*4
    assert MA.active_volume_ask == 1
    assert MA.active_volume_bid == 5
    # 
    order_list = MA.clear_position(LOB, time=4)
    reward = MA.update_position_from_message_list(order_list)
    # 
    assert MA.inventory == 0
    assert MA.active_volume_ask == 0
    assert MA.active_volume_bid == 0
...
def test_market_making_multiple_fills():
    MA = MarketMakingAgent(volume=20, agent_id='market_making_agent')
    LOB = LimitOrderBook(['market', 'market_making_agent'])
    orders = []
    orders.append(LimitOrder(side='bid', agent_id='market', price=100, volume=10, time=0))
    orders.append(LimitOrder(side='ask', agent_id='market', price=101, volume=10, time=1))
    ...
    orders = []
    orders.append(LimitOrder(side='ask', agent_id='market_making_agent', price=101, volume=5, time=2))
    orders.append(LimitOrder(side='ask', agent_id='market_making_agent', price=102, volume=10, time=3))
    orders.append(LimitOrder(side='bid', agent_id='market_making_agent', price=99, volume=5, time=4))
    msg_list = [LOB.process_order(order) for order in orders]
    MA.update_position_from_message_list(msg_list)
    # 
    assert MA.active_volume_ask == 15
    assert MA.active_volume_bid == 5
    assert MA.inventory == 0
    # market buy order. should clear the market making order 
    order = MarketOrder(side='ask', agent_id='market', volume=10+5+5, time=5)
    fill = LOB.process_order(order)
    reward = MA.update_position_from_message_list([fill])
    # fill on the ask side (limit sell order)
    assert MA.inventory == -10
    assert reward == 101*5 + 102*5  
    # fill of limit sell at this price  
    assert MA.active_volume_ask == 5
    assert MA.active_volume_bid == 5
    return None
...
if __name__ == '__main__':
    test_market_making_agent_limit_fills()
    test_making_agent_cancellations()
    test_market_making_multiple_fills()
    ####
    test_market_order()
    test_limit_order()
