''''
 - this file contains different types of agents used in the simulation
...
class MarketMakingAgent():
    """
        - keeps track of the 
            - volume 
            - active_volume (placed in the order book)
            - active_volume_bid and active_volume_ask (placed in the order book on bid and ask side)
            - inventory (total number bought - total number sold)
            - PnL (sum of cash from sells - sum of cash from buys. in this case its always better to sell)

        - reset function (starts everything again from the beginning)

        - update_position: updates the agents position from a fill message --> update volumes, inventory, pnl, rewards
        
        - questions: 
            - one issue: just selling everything leads to high pnl 
            - other things to include mark-to-market of inventory at mid-price 
            - inventory PnL 
            - spread capture

        TODO: implement no self-matching logic. for now just work with limit orders on both sides of the book 
        
        can we introduce adverse price jumps, to make things more difficult? 
    """
    def __init__(self, volume, agent_id, priority=0) -> None:
        # maximal volume that the algorithm is able to place 
        # we are assuming lot sizes of one lot - so this is the same as the number of lots 
        # at each time step, decide how to distribute this volume accross price levels 
    ...
    def reset(self):
        self.inventory = 0
        self.active_volume_bid = 0
        self.active_volume_ask = 0
        self.pnl = 0
    ...
    def update_position(self, fill_message):        
        assert self.active_volume_bid >= 0
        assert self.active_volume_ask >= 0
        # we impose the no more orders than max volume can be posted in the book 
        assert self.active_volume_bid + self.active_volume_ask <= self.max_volume
        reward = 0
        if fill_message.type == 'modification':
            ...
            # cancellation by price and volume could also return a list of cancellations and modifications
            if fill_message.order.agent_id == self.agent_id:
                if fill_message.order.side == 'bid':
                    self.active_volume_bid -= fill_message.filled_volume
                elif fill_message.order.side == 'ask':
                    self.active_volume_ask -= fill_message.filled_volume
                else:
                    raise ValueError(f'Unknown order side {fill_message.order.side}')
            else:
                pass
            ...
            # this means the limit order was send into the book by the agent 
                if fill_message.side == 'bid':
                    self.active_volume_bid += fill_message.volume
                elif fill_message.side == 'ask':
                    self.active_volume_ask += fill_message.volume
                else:
                    raise ValueError(f'Unknown order side {fill_message.order.side}')
        elif fill_message.type == 'cancellation':
            # cancellation does not have agent_id attribute
            # should change this 
            if fill_message.order.agent_id == self.agent_id:
                if fill_message.side == 'bid':
                    self.active_volume_bid -= fill_message.volume
            ...
            # rewards can only be generated through market orders. either passsive or active fills.
            side = fill_message.order.side
            if fill_message.order.agent_id == self.agent_id:
                # market order that was send by the agent 
                if side == 'ask':
                    # market buy --> volume increases, negative cash flow 
                    # execution price is fill_price * filled_volume. but its positive 
                    self.inventory += fill_message.filled_volume
                    reward = reward - fill_message.execution_price 
                elif side == 'bid':
                    # market sell: the volume decreases, which leads to positive cash flow 
                    self.inventory -= fill_message.filled_volume
                    reward = reward + fill_message.execution_price
            # passive fills 
            if self.agent_id in fill_message.passive_fills:
                # assert side == 'ask', 'this execution agent only sells'
                # market making agent buys and sells 
                cash = 0
                volume = 0 
                ...
                # TODO: should improve the wording of bid and ask 
                if side == 'ask':
                    # market buy order --> limit sell fill 
                    self.inventory -= volume
                    self.active_volume_ask -= volume
                    # selling means positive cash flow
                    reward = reward + cash 
                elif side == 'bid':
                    # market sell --> limit buy
                    self.inventory += volume
                    self.active_volume_bid -= volume
                    reward = reward - cash
        else: 
            raise ValueError(f'Unknown message type {fill_message.type}')
        return reward
    
    def clear_position(self, lob, time):
        ...
        if self.inventory > 0:
            # we have a long position --> market sell order 
            order_list.append(MarketOrder(agent_id=self.agent_id, side='ask', volume=self.inventory, time=time))
        elif self.inventory < 0:
            # we have a short position --> market buy order
            order_list.append(MarketOrder(agent_id=self.agent_id, side='bid', volume=abs(self.inventory), time=time))
        else:
            pass
        return order_list

class TOPAgent(MarketMakingAgent):
    '''
        - places limit orders at the top of the book (best bid and best ask)
        - always places a fixed amount of limit orders on both sides 
    '''
    def __init__(self, volume, agent_id='TOP', priority=0) -> None:
        super().__init__(volume, agent_id, priority)
        assert volume > 0, 'volume must be positive for TOP agent'
        assert isinstance(volume, int), 'volume must be integer for TOP agent'
        assert volume % 2 == 0, 'volume must be even for TOP agent'
        self.initial_time = 0 

    def generate_order(self, lob, time):
        # TODO: implement termnal liquidation 
        # update_position_from_message_list should return terminated on final time 
        assert self.active_volume_bid >= 0
        assert self.active_volume_ask >= 0
        assert self.active_volume_bid + self.active_volume_ask <= self.max_volume
        assert 0 <= time 
        ...
        best_bid_price = lob.get_best_price('bid')        
        best_ask_price = lob.get_best_price('ask')

        # check for orders on bid and ask side of the book 
        volume_on_best = 0 
        volume_on_ask = 0 
        orders_on_best = set()
        for order in lob.order_map_by_agent[self.agent_id]:
            if lob.order_map[order].side == 'bid':
                if lob.order_map[order].price == best_bid_price:
                    volume_on_best += lob.order_map[order].volume
                    orders_on_best.add(order)
            elif lob.order_map[order].side == 'ask':
                if lob.order_map[order].price == best_ask_price:
                    volume_on_ask += lob.order_map[order].volume
                    orders_on_best.add(order)
            else:
                raise ValueError(f'Unknown order side {lob.order_map[order].side}')
        
        # cancel order not on beset 
        not_on_best = lob.order_map_by_agent[self.agent_id] - orders_on_best                
        order_list = []
        for order_id in not_on_best:
            order = lob.order_map[order_id]
            order_list.append(Cancellation(agent_id=self.agent_id, order_id=order_id, time=time))
        
        # redo allocations on bid and ask side 
        target_volume_per_side = self.max_volume/2
        assert 0 <= volume_on_best <= target_volume_per_side, 'volume on best must be between 0 and max volume'
        if volume_on_best < target_volume_per_side:
            order_list.append(LimitOrder(agent_id=self.agent_id, side='bid', price=best_bid_price, volume=target_volume_per_side - volume_on_best, time=time))        
        assert 0 <= volume_on_ask <= target_volume_per_side, 'volume on ask must be between 0 and max volume'
        if volume_on_ask < target_volume_per_side:
            order_list.append(LimitOrder(agent_id=self.agent_id, side='ask', price=best_ask_price, volume=target_volume_per_side - volume_on_ask, time=time))
        
        return order_list

    def initial_event(self):
        return (self.initial_time, self.priority, self.agent_id)

    def next_event(self, time):
        # how to implement stopping at terminal time ?
...
class LimitAgent(ExecutionAgent):

    def __init__(self, volume, start_time, priority, level) -> None:
        super().__init__(volume, 'limit_agent', priority)
        self.start_time = start_time
        # level starts from 1
        self.level = level
        # placing at the best ask + level -1 
    ...
    def generate_order(self, lob, time):
        ...
        if time == self.start_time:
            self.reference_bid_price = lob.get_best_price('bid')
            limit_price = lob.get_best_price('ask')+self.level-1
...
class RLAgent(ExecutionAgent):
    ...
    def get_observation(self, time, lob):        
        ...
        if self.drop_feature == 'order_info':
            # dropping order info features
            pass
        else:
            observation = np.concatenate([observation, order_dist, levels, queues], dtype=np.float32)
        
        # drift features
        if self.drop_feature == 'drift':
            # dropping more drift features
            pass
        else:
