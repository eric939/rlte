...
class Market(gym.Env):
    ...
    def __init__(self, config):
        ...
        # flow or strategic 
            ...
            #     # undo this for most tests. 
            #     # test out furher reduction in intensity scaling here. 
            #     intensity_scaling = noise_agent_config['intensity_scaling']
            #     agent.market_intensity = agent.market_intensity * 0.8
            #     agent.cancel_intensities = agent.cancel_intensities * 0.8
            #     # self.agents[agent.agent_id] = agent
            # else: 
...
def rollout(seed, n_episodes, execution_agent, market_type, volume, terminal_time, time_delta):
    ...
    config = {'seed': seed, 'market_env': market_type, 'execution_agent': execution_agent, 'volume': volume, 'terminal_time': terminal_time, 'time_delta': time_delta, 'drop_feature': None}
    M = Market(config)
    total_rewards = []
    times = []
    n_events = []
...
if __name__ == '__main__':
    ...
    for env in envs:
        for lots in n_lots:
            for agent in agents:
                ...
                rewards, times, n_events = mp_rollout(n_samples=n_samples, n_cpus=n_cpus, execution_agent=agent, market_type=env, volume=lots, seed=seed, terminal_time=150, time_delta=30)
                file_name = f'rewards/{env}_{lots}_episodes_{n_samples}_eval_seed_{seed}_{agent}_long_horizon.npz'
                print('savin file to ', file_name)
    ...
    # # rollout 
    # start_time = time.time()
    # rewards, times, n_events = rollout(seed=0, n_episodes=n_samples, execution_agent=agent, market_type=env, volume=lots, terminal_time=300, time_delta=30)
    # end_time = time.time()
    # execution_time = end_time - start_time
    # print("Execution time:", execution_time)
